...
export function TeacherChatPage() {
  ...
  useEffect(() => {
    chatsApi.getChats().then((r) => {
      const st = r.data.filter((c) => c.type === 'STUDENT_TEACHER');
      st.forEach((c) => setUnread(c.id, c.unread));
      if (isTeacherOrAdmin) {
        setChats(st);
      } else {
        // Student: find their own ST chat
        setSelected(st[0] ?? null);
      }
    });
  }, []);
            ...
            <div className="w-56 shrink-0 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl overflow-y-auto">
              ...
              {chats.map((c) => (
                ...
                >
                  ...
                  {c.unread > 0 && (
                    <span className="text-xs bg-red-50 dark:bg-red-900/200 text-white rounded-full px-1.5 py-0.5">{c.unread}</span>
                  )}
                </button>
              ))}
            </div>
